...
for i, thresh in enumerate(thresholds):
    ...
    if red_pixel_count > red_pixel_threshold:
        ...
        while True:
            # クリックされた領域の色を変えずに矩形を再描画
            temp_image = image_with_detections.copy()
            for x1, y1, x2, y2, clicked in red_lights:
                if clicked:
                    cv2.rectangle(temp_image, (x1, y1), (x2, y2), (255, 0, 0), 2)  # 青色で上書き

            cv2.imshow(f"Detected Red Lights with Threshold {i + 1}", temp_image)
            key = cv2.waitKey(1)
            if key == 27:  # Escキーで終了
                break
